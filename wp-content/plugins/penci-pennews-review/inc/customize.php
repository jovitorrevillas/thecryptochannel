...
if( ! function_exists( 'penci_soledad_review_customizer' ) ) {
	function penci_soledad_review_customizer( $wp_customize ) {
		...
		$wp_customize->add_setting( 'penci_review_process_total_color', array(
			'default'           => '#ffffff',
			'sanitize_callback' => 'sanitize_hex_color'
		) );
		$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'penci_review_process_total_color', array(
			'label'    => 'Review Average Total Process Main Background Color',
			'section'  => 'penci_new_section_review',
			'settings' => 'penci_review_process_total_color',
			'priority' => 4
		) ) );

		$wp_customize->add_setting( 'review_process_total_run_color', array(
			'default'           => '#fffff',
			'sanitize_callback' => 'sanitize_hex_color'
		) );
		$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'review_process_total_run_color', array(
			'label'    => 'Review Average Total Process Running Background Color',
			'section'  => 'penci_new_section_review',
			'settings' => 'review_process_total_run_color',
			'priority' => 4
		) ) );
...
if( ! function_exists( 'penci_review_customizer_css' ) ) {
	function penci_review_customizer_css() {
		?>
		<style type="text/css">
			<?php if(get_theme_mod( 'penci_review_border_color' )) : ?>.wrapper-penci-review, .penci-review-container.penci-review-count { border-color:<?php echo get_theme_mod( 'penci_review_border_color' ); ?>; }<?php endif; ?>
			...
			<?php if(get_theme_mod( 'penci_review_desc_color' )) : ?>.penci-review-container .penci-review-desc p { color:<?php echo get_theme_mod( 'penci_review_desc_color' ); ?>; }<?php endif; ?>
			...
			<?php if(get_theme_mod( 'penci_review_process_total_color' )) : ?>.penci-review-average .penci-review-process { background-color:<?php echo penci_convert_hex_rgb( get_theme_mod( 'penci_review_process_total_color' ), $opacity = 0.26 ) ; ?>; }<?php endif; ?>
			<?php if(get_theme_mod( 'review_process_total_run_color' )) : ?>.penci-review-average .penci-review-process span.penci-process-run { background-color:<?php echo get_theme_mod( 'review_process_total_run_color' ); ?>; }<?php endif; ?>
