...
if( ! class_exists( 'Pennew_Portfolio_Helper' ) ) {
	class Pennew_Portfolio_Helper{
		...
		public static function get_html_pagination( $custom_query = false, $atts = array(), $data_query = array() ) {
			...
			$number = isset( $data_query['size'] ) ? intval( $data_query['size'] ) : '';
			...
			if ( in_array( $atts['style_pag'], array( 'load_more', 'infinite' ) ) ) {
				printf(
					...
					$atts['style_pag'],
					'penci-plf-loading-2',
					function_exists( 'penci_get_tran_setting' ) ?  penci_get_tran_setting( 'penci_click_handle_text' ) : esc_html__( 'Load More', 'penci-framework' ),
		...
		public static function get_image_ratio( $image, $echo = true ) {
			...
			if ( preg_match_all( '#(width|height)=(\'|")?(?<dimensions>[0-9]+)(\'|")?#i', $image, $image_dis ) && 2 == count( $image_dis['dimensions'] ) ) {
