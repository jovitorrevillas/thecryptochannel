...
class Penci_Helper_Shortcode {
	...
	public static function get_pull_down_filter( $atts, $shortcode_id, $unique_id, $show = true ) {
		...
		foreach ( $items as $key => $item ) {
			$html_item = '<li class="penci-subcat-item"><a class="penci-subcat-link ' . ( $key < 1 ? 'active penci-subcat-item-1' : '' ) . '" data-filter_value="' . $item['id'] . '" data-block_id="' . $unique_id . '" href="#">' . do_shortcode(  $item['name'] ) . '</a></li>';
	...
	public static function get_image_holder_gal( $attach_id, $size = 'full', $is_background = true ) {
		...
		if ( $is_background ) {
			$output = sprintf( '<a class="penci-image-holder penci-lazy" data-src="%s" href="%s" title="%s"></a>',
				$src_thmb, $src_large, self::get_image_alt( $attach_id )
			);
		} else {
			$output = sprintf( '<a class="owl-lazy" href="%s" title="%s"><img src="%s" alt="%s" /></a>',
				$src_large, self::get_image_alt( $attach_id ), $src_thmb, self::get_image_alt( $attach_id )
			);
		}
	 ...
	 * @return string
	 */
	public static function get_post_meta( $args, $atts, $wapper = true ) {
		$output = '';
	...
	public static function get_excrept( $length, $hide_excrept ) {
		...
		$excrept  = wp_trim_words( $content, $length, '' );
		$excrept .= '...';

		return ( $excrept && empty( $hide_excrept ) ? '<div class="penci-post-excerpt">' . $excrept . '</div>' : '' );
	}

	public static function get_markup_title_post( $title_length ) {
		$post_id = get_the_ID();
		 return  '<h3 class="penci__post-title entry-title"><a href="' . get_the_permalink( $post_id ) . '" title=" ' . get_the_title( $post_id ) . ' ">' . penci_trim_post_title( $post_id, $title_length ) . '</a></h3>';
	}
	...
	public static function get_image_holder2( $image_size, $class = '', $show_icon = false, $class_icon = '', $item_order = '' ) {
		...
		$output = sprintf( '<a class="penci-image-holder penci-lazy%s%s" data-src="%s" data-delay="%s" href="%s" title="%s">%s<span class="screen-reader-text">%s</span></a>',
			$class ? ' ' . $class : '',
			function_exists( 'penci_icon_post_format' ) && $show_icon ? ' penci-image_has_icon' : '',
			Penci_Framework_Helper::get_featured_image_size( get_the_ID(), $image_size ),
	...
	public static function get_image_holder( $args ) {
		...
		$output = sprintf( '<a class="penci-image-holder %s%s%s" data-src="%s" data-delay="" href="%s" title="%s">%s</a>',
			$use_penci_lazy ? 'penci-lazy' : '',
			$class ? ' ' . $class : '',
			function_exists( 'penci_icon_post_format' ) && $show_icon ? ' penci-image_has_icon' : '',
			Penci_Framework_Helper::get_featured_image_size( get_the_ID(), $image_size ),
			get_the_permalink(),
	...
	public static function get_image_holder_pre( $args ) {
		...
		$output = sprintf( '<a class="penci-image-holder penci-lazy %s" data-src="%s" data-delay="" href="%s" title="%s">%s %s<span class="screen-reader-text">%s</span></a>',
			$class,
			Penci_Framework_Helper::get_featured_image_size( get_the_ID(), $image_size ),
			get_the_permalink(),
			wp_strip_all_tags( get_the_title() ),
	...
	public static function get_html_animation_loading(){
		...
		$style_animation = get_theme_mod( 'penci_general_loader_effect' ) ? get_theme_mod( 'penci_general_loader_effect' ) : 6;
		...
		return isset( $animation[$style_animation] ) ? $animation[$style_animation] : $animation[6];
	}
	...
	public static function get_general_css_custom( $id, $atts ) {
		...
		if( 'style-title-9' == $style_block_title ) {
			...
			if( $border_left_right_color ) {
				...
				$css .= sprintf( '%s.style-title-9.style-title-right .penci-block-heading{ border-right-color:%s !important; }', $id, $border_left_right_color );
		...
		if( $css && ! empty( $atts['style_pag'] ) ){
			$background =  self::pre_background_design_vc( $css );
		...
		if( $css && 'style-title-11' == $style_block_title ){
			$background =  self::pre_background_design_vc( $css );
