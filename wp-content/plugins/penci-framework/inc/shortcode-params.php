...
class Penci_Framework_Shortcode_Params {
	...
	public static function block_build_query( $size = 5 , $setting = array() ) {
	 ...
	 * @return array
	 */
	public static function block_option_meta( $args ) {
		if ( empty( $args ) ) {
			return array();
		}
		...
		$ajaxfilter = array(
			array(
				...
				'value'       => array(
					...
					//esc_html__( 'Filter by authors', 'penci-framework' )            => 'author_ids_filter',
					esc_html__( 'Filter by tag slug', 'penci-framework' )           => 'tag_slug_filter',
				),
				'description' => esc_html__( 'Show the ajax drop down filter. The ajax filters (except by popularity) require an additional parameter. If no ids are provided in the input below, the filter will show all the available items (ex: all authors, all categories etc..)', 'penci-framework' ),
			...
			array(
				...
				'std'         => '',
				'group'       => $group_ajaxfilter,
				'description' => esc_html__( 'The ajax drop down shows only the (author ids, categories ids OR tag slugs) that you enter here separated by comas', 'penci-framework' ),
			),
			array(
				'type'        => 'textfield',
