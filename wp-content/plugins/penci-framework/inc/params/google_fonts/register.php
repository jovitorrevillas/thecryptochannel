...
function penci_vc_param_google_fonts( $settings, $value ) {
	...
	foreach ( $fonts as $font_data ) {
		$output .= '<option value="' .  $font_data->font_family . ':' . $font_data->font_styles  . '"';
		$output .= 'data[font_types]="' . $font_data->font_types . '"';
		$output .= ' data[font_family]="' . $font_data->font_family . '"';
		$output .= 'data[font_styles]="' .  $font_data->font_styles . '"';
		$output .= 'class="' . vc_build_safe_css_class( $font_data->font_family ) . '"';
		$output .= ( strtolower( $values['font_family'] ) == strtolower( $font_data->font_family ) || strtolower( $values['font_family'] ) == strtolower( $font_data->font_family ) . ':' . $font_data->font_styles ? 'selected' : '' );
		$output .= '>' . $font_data->font_family . '</option>';
	}
	$output .= '</select>';
	$output .= '</div>';
	$output .= '</div>';
