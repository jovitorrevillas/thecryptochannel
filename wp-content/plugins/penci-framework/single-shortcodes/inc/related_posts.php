	...
	function penci_related_posts_shortcode( $atts, $content ) {
		...
		while ( $the_query->have_posts() ) {
			...
			if ( 'grid' == $atts['style'] ) {
				...
				$image_type = ( get_theme_mod( 'penci_archive_image_type' ) ? get_theme_mod( 'penci_archive_image_type' ) : 'landscape' );

				$output .= '<li  class="penci-post-item penci-imgtype-' . esc_attr( $image_type ) . '">';
				$output .= '<div class="penci_media_object ' . ( 'right' == $atts['thumbright'] ? 'penci_mobj-image-right' : '' ) . '">';
