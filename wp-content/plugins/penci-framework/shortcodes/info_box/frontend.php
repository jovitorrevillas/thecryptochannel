...
$class .= $atts['background_style'] ? ' penci-ibox-' . $atts['background_style'] : '';
$class .= $atts['icon_size'] ? ' penci-ibox-size-' . $atts['icon_size'] : 'penci-ibox-size-md';
$class .= $atts['css'] ? ' ' . vc_shortcode_custom_css_class( $atts['css'], '' ) : '';
...
<div id="<?php echo esc_attr( $unique_id ); ?>" class="<?php echo esc_attr( $class ); ?>">
	<div class="penci-ibox-inner">
		...
		if ( 'icon' == $atts['icon_type'] ) {
			...
			$icon .= $a_before;
			$icon .= '<img class="penci-ibox-img_active" src="' . esc_url( wp_get_attachment_url( $atts['image'] ) ) . '">';
	...
	if ( $atts['icon_hborder_color'] ) {
		...
		$css_custom .= $id_iconbox . '.penci-ibox-background.penci-ibox-effect-s2 .penci-ibox-icon--i:after{  box-shadow: 0 0 0 2px ' . ( $atts['icon_hborder_color'] ) . '; }';
	}
