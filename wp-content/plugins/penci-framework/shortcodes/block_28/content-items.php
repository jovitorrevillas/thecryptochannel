...
while ( $query_slider->have_posts() ) {
	...
	if ( $block28_i == 1 && ( ! isset( $styleAction ) || (  isset( $styleAction ) && 'load_more' != $styleAction ) ) ) {
		...
		$block28_first_item .= Penci_Helper_Shortcode::get_post_meta( array( 'author', 'date', 'comment' ), $atts );
		...
		$block28_items .= empty( $atts['hide_cat'] ) ? '<div class="penci-post-cat">' . Penci_Framework_Helper::show_category( '', $atts['category_ids'], false ) . '</div>' : '';
		$block28_items .= Penci_Helper_Shortcode::get_markup_title_post(  $atts['post_standard_title_length'] );
		$block28_items .= Penci_Helper_Shortcode::get_post_meta( array( 'date', 'comment','view' ), $atts );;
