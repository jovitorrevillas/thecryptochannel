...
while ( $query_slider->have_posts() ) {
	...
	if ( $block17_i < 3 && ( ! isset( $styleAction ) || (  isset( $styleAction ) && 'load_more' != $styleAction ) )  ) {
		...
		$block17_items .= Penci_Helper_Shortcode::get_post_meta( array( 'date', 'comment','view' ), $atts );

		$block17_items .= '</div></article>';
		...
		$block17_items .= '<article  class="' . join( ' ', penci_get_post_class( 'penci-post-item-' . $block17_i, get_the_ID() ) ) . '">';
