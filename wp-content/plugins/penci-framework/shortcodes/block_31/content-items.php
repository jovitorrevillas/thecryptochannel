...
while ( $query_slider->have_posts() ) {
	...
	if(  1 == $block31_i && ( $atts['turn_on_loop_item'] || ! isset( $styleAction ) || ( isset( $styleAction ) && 'load_more' != $styleAction ) ) ) {
		...
		$block30_items .= '<article  class="' . join( ' ', penci_get_post_class( 'block31_big_item', get_the_ID() ) ) . '">';
		...
		$block30_items .= empty( $atts['hide_cat'] ) ?  '<div class="penci-post-cat">'. Penci_Framework_Helper::show_category('',$atts['category_ids'], false ) . '</div>' : '';
		...
		$block30_items .= Penci_Helper_Shortcode::get_post_meta( array( 'date','comment', 'review','view' ), $atts );
		...
		$block30_items .= '<article  class="' . join( ' ', penci_get_post_class( 'block31_medium_item', get_the_ID() ) ) . '">';
		...
		$block30_items .= Penci_Helper_Shortcode::get_post_meta( array( 'date','comment','view' ), $atts );;
