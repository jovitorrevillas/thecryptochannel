...
if ( in_array( $atts['style_slider'], array( 'style-1', 'style-7', 'style-14', 'style-27' ) ) ) {
	$data_autowidth = 1;
}
...
if ( in_array( $atts['style_slider'], array( 'style-12', 'style-13', 'style-14' ) ) ) {
	echo '<div id="' . esc_attr( $unique_id ) . '" class="penci-slider-sync penci-owl-featured-area penci-fadeInUp' . esc_attr( implode( $class, ' ' ) ) . '" ' . $data . '>';
	include dirname( __FILE__ ) . "/{$style_slider}.php";
	echo '</div>';
} else {
	echo '<div id="' . esc_attr( $unique_id ) . '" class="penci-owl-carousel-slider penci-owl-featured-area  penci-owl-carousel-style' . esc_attr( implode( $class, ' ' ) ) . '" ' . $data . '>';
	include dirname( __FILE__ ) . "/{$style_slider}.php";
	echo '</div>';
}
...
if ( in_array( $atts['style_slider'], array( 'style-12','style-13','style-14' ) ) ) {
	...
	if ( $atts['meta_hcolor'] ) {
		$css_custom .= sprintf( '%s .penci-slider__meta a:hover{ color:%s !important;  }', $id_slider, $atts['meta_hcolor'] );
	}
}
...
if ( 'style-23' == $atts['style_slider'] ) {

	if ( $atts['slider_overlay_bg'] ) {
