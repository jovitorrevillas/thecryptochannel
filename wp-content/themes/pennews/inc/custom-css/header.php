...
function penci_customizer_header() {
	$padding_top_logo      = get_theme_mod( 'penci_logo_padding_top' );
	$padding_bottom_logo   = get_theme_mod( 'penci_logo_padding_bottom' );
	$width_logo            = get_theme_mod( 'penci_logo_width' );
	$header_layout         = penci_get_setting( 'penci_header_layout' );
	$main_menu_line_height = get_theme_mod( 'penci_main_menu_line_height' );
	$css                   = '';
	...
	if( $padding_top_logo || 0 === $padding_top_logo  ){
		...
		if( in_array( $header_layout, array( 'header_s3','header_s4','header_s6' ) ) ) {
			...
			$css .= sprintf( '.site-header .site-branding a,.header__top .site-branding a{ top:%spx !important; }',
				esc_attr( $padding_top_logo )
			);
		}
	}
	...
	if ( $width_logo && $width_logo < 400 ) {
		...
		if ( 'header_s1' == $header_layout ) {
			$css .= sprintf( '.header--s1 .site-branding,.header--s1 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s2' == $header_layout ) {
			$css .= sprintf( '.header--s2 .site-branding,.header--s2 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s5' == $header_layout ) {
			$css .= sprintf( '.header--s5 .site-branding,.header--s5 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s7' == $header_layout ) {
			$css .= sprintf( '.header--s7 .site-branding,.header--s7 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s8' == $header_layout ) {
			$css .= sprintf( '.header--s8 .site-branding,.header--s8 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		}elseif ( 'header_s9' == $header_layout ) {
			$css .= sprintf( '.header--s9 .site-branding,.header--s8 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s10' == $header_layout ) {
			$css .= sprintf( '.header--s10 .site-branding,.header--s10 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		} elseif ( 'header_s11' == $header_layout ) {
			$css .= sprintf( '.header--s11 .site-branding,.header--s11 .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		}else{
			$css .= sprintf( '.header__top .site-branding,.header__top .site-branding img {width: %spx !important;overflow: hidden; }', esc_attr( $width_logo ) );
		}
	}

	// Slogan
	$css_slogan = '';
	$font_slogan        = get_theme_mod( 'penci_font_slogan' );
	$font_weight_slogan = get_theme_mod( 'penci_font_weight_slogan' );
	$font_style_slogan  = get_theme_mod( 'penci_font_style_slogan' );
	$fontsize_slogan    = get_theme_mod( 'penci_fontsize_slogan' );
	$padding_top_slogan = get_theme_mod( 'penci_padding_top_slogan' );
	...
	// Main menu
	$font_main_menu_item           = get_theme_mod( 'penci_font_main_menu_item' );
	$font_weight_menu_item         = get_theme_mod( 'penci_font_weight_menu_item' );
	$font_size_menu_lever1         = get_theme_mod( 'penci_font_size_menu_lever1' );
	$font_size_menu_dropdown       = get_theme_mod( 'penci_font_size_menu_dropdown' );
	$font_size_child_cat_mega      = get_theme_mod( 'penci_font_size_child_cat_mega' );
	$font_size_ptitle_cat_mega     = get_theme_mod( 'penci_font_size_ptitle_cat_mega' );
	$font_size_pdate_cat_mega      = get_theme_mod( 'penci_font_size_pdate_cat_mega' );
	$hide_uppercase_menu_item      = get_theme_mod( 'penci_hide_uppercase_menu_item' );
	$hide_uppercase_chid_mega_menu = get_theme_mod( 'penci_hide_uppercase_chid_mega_menu' );
	...
	if ( $font_main_menu_item || $font_for_body != penci_default_setting( 'penci_font_for_body' ) ) {
		$css .= sprintf( '.main-navigation a,.mobile-sidebar .primary-menu-mobile li a{ font-family: %s; }',
			penci_google_fonts_parse_attributes( $font_main_menu_item ) );
	}

	if ( $font_weight_menu_item ) {
		$css .= sprintf( '.main-navigation a,.mobile-sidebar .primary-menu-mobile li a{ font-weight: %s; }',
	...
	if( get_theme_mod( 'penci_hide_arrow_down_menu_item' ) ) {
	...
	if( get_theme_mod( 'penci_hide_arrow_right_menu_item' ) ) {
	...
	if( get_theme_mod( 'penci_hide_line_top_dropdown_menu' ) ) {
	...
	$ajaxsearch_bg_color         = get_theme_mod( 'penci_ajaxsearch_bg_color' );
	$ajaxsearch_title_color      = get_theme_mod( 'penci_ajaxsearch_title_color' );
	$ajaxsearch_title_hcolor     = get_theme_mod( 'penci_ajaxsearch_title_hcolor' );
	$ajaxsearch_postmeta_color   = get_theme_mod( 'penci_ajaxsearch_postmeta_color' );
	$ajaxsearch_border_color     = get_theme_mod( 'penci_ajaxsearch_border_color' );
	$ajaxsearch_border_top_color = get_theme_mod( 'penci_ajaxsearch_border_top_color' );

	if ( $ajaxsearch_bg_color ) {
		$css .= sprintf( '#top-search .show-search .show-search__content{ background: %s; }', esc_attr( $ajaxsearch_bg_color ) );
	}

	if ( $ajaxsearch_title_color ) {
		$css .= sprintf( '#top-search .penci-viewall-results a,#top-search .penci-ajax-search-results-wrapper .penci__post-title a,
		.penci_dark_layout #top-search .penci-viewall-results a, .penci_dark_layout #top-search .penci-ajax-search-results-wrapper .penci__post-title a{ color: %s; }', esc_attr( $ajaxsearch_title_color ) );
	}

	if ( $ajaxsearch_title_hcolor ) {
		$css .= sprintf( '#top-search .penci-viewall-results a:hover,#top-search .penci-ajax-search-results-wrapper .penci__post-title a:hover,
		.penci_dark_layout #top-search .penci-viewall-results a:hover, .penci_dark_layout #top-search .penci-ajax-search-results-wrapper .penci__post-title a:hover{ color: %s; }', esc_attr( $ajaxsearch_title_hcolor ) );
	}

	if ( $ajaxsearch_postmeta_color ) {
		$css .= sprintf( '#top-search .penci-ajax-search-results-wrapper .penci__general-meta .penci_post-meta{ color: %s; }', esc_attr( $ajaxsearch_postmeta_color ) );
	}
