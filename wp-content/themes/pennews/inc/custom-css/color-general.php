...
function penci_customizer_color_general() {
	...
	if( get_theme_mod( 'penci_dis_padding_block_widget' ) ) {
	...
	if( get_theme_mod( 'background_color' ) ) {
		...
		.penci_dis_padding_bw .penci-block-vc.style-title-11:not(.footer-widget) .penci-slider-nav{ background-color:#%s; }',  get_theme_mod( 'background_color' ) );
	}

	$color_text_body   = get_theme_mod( 'penci_color_body' );
	$penci_colorscheme = get_theme_mod( 'penci_colorscheme' );
	...
	$block_bgcolor = get_theme_mod( 'penci_block_bgcolor' );
	if ( $block_bgcolor ) {
		$css .= sprintf( '
		.penci-ajax-search-results .ajax-loading:before,
		.show-search .show-search__content,
		div.bbp-template-notice, div.indicator-hint,	
		.widget select,select, pre,.wpb_text_column,#wp-calendar tbody td,
	...
	$color_meta = get_theme_mod( 'penci_color_meta' );
	if ( $color_meta ) {
	...
	$color_links = get_theme_mod( 'penci_color_links' );
	...
	$color_heading = get_theme_mod( 'penci_color_heading' );
	if ( $color_heading ) {
		$css .= sprintf( '
		h1, h2, h3, h4, h5, h6,
	...
	$color_border = get_theme_mod( 'penci_color_border' );
	if ( $color_border ) {
	...
	$color_accent = get_theme_mod( 'penci_color_accent' );
	...
	if ( $color_accent ) {
		...
		$css .= sprintf(
			...
			'button:hover,.button:hover, .entry-content a.button:hover, input[type="button"]:hover,input[type="reset"]:hover,input[type="submit"]:hover,' .
