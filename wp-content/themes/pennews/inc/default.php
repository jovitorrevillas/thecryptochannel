...
function penci_default_setting( $name ) {
	...
	$defaults = array(
		...
		'penci_footer_copyright' => sprintf( '@2018 - PenciDesign. All Right Reserved. Designed and Developed by <a href="%s" target="_blank">PenciDesign</a>',
			esc_url( 'https://themeforest.net/item/news-wordpress-theme-focus-only-for-magazinenews-pennews/20822517?ref=PenciDesign' ) ),
		...
		'penci_hide_page_share_stumbleupon'   => 1,
		'penci_hide_page_share_whatsapp'      => 1,
...
if ( ! function_exists( 'penci_get_setting' ) ):
	function penci_get_setting( $name , $type = '' ) {
		...
		if( 'color' == $type ){
			$value = get_theme_mod( $name, penci_colors_default_setting( $name ) );
		}else{
			$value = get_theme_mod( $name, penci_default_setting( $name ) );
		}
