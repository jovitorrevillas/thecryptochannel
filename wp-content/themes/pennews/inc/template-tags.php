...
function penci_get_categories() {
	...
	if( get_theme_mod( 'archive_show_pri_cat' )&& class_exists( 'WPSEO_Primary_Term' ) ) {
...
add_filter( 'comment_form_fields', 'penci_move_comment_field_to_bottom' );
 ...
 * @return mixed
 */
function penci_move_comment_field_to_bottom( $fields )
{
...
if( !function_exists( 'penci_get_post_countview' ) ) {
	function penci_get_post_countview( $post_id, $show = false ) {
		...
		$output .= '<i class="fa fa-eye"></i><span class="penci-post-countview-number">' . $count . '</span>';
		$output .= '</span>';
