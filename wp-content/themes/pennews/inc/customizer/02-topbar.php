...
$wp_customize->add_control( new WP_Customize_Control(
	...
	array(
		'label'    => esc_html__( 'Show the top menu', 'pennews' ),
		'section'  => $section_topbar,
		'type'     => 'checkbox',
...
$wp_customize->add_setting( 'penci_topbar_menu', array(
	'default'           => '',
	'sanitize_callback' => array( $sanitizer, 'select' ),
) );
$wp_customize->add_control( 'penci_topbar_menu', array(
	'label'    => esc_html__( 'Select the top menu', 'pennews' ),
	'section'  => $section_topbar,
	'settings' => 'penci_topbar_menu',
	'type'     => 'select',
	'choices'  => penci_get_user_created_menus()
) );
...
$wp_customize->add_control( 'penci_topbar_location_weather', array(
	...
	'description' => sprintf( '%s - You can use "city name" (ex: London) or "city name,country code" (ex: London,uk)%s',
		...
		'</br><a target="_bank" href="' . esc_url( 'https://image.prntscr.com/image/7WsPGQsATS6yT6TF1Bt8gQ.png' ) . '"><img alt="" src="' . esc_url( 'https://image.prntscr.com/image/7WsPGQsATS6yT6TF1Bt8gQ.png' ) . '"/></a>'
	)
) );
