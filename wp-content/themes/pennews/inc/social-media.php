...
function penci_list_socail_media( $show = true ) {
	...
	foreach ( $socials as $id => $social ) {
		...
		$output .= sprintf( '<a class="social-media-item socail_media__%s" target="_blank" href="%s" title="%s"><span class="socail-media-item__content"><i class="fa fa-%s"></i><span class="social_title screen-reader-text">%s</span></span></a>',
			$id, $url,$social, $icon, $social );
	}
